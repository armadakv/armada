...
// an example.
func CreateFlagFile(dir string, filename string, msg pb.Marshaler, fs vfs.FS) (err error) {
...
// protobuf message.
func GetFlagFileContent(dir string, filename string, msg pb.Unmarshaler, fs vfs.FS) (err error) {
	fp := fs.PathJoin(dir, filename)
...
func TempFile(dir,
	pattern string, fs vfs.FS,
) (f vfs.File, name string, err error) {
	if dir == "" {
		dir = vfs.TempDir()
		if fs != vfs.Default {
			if err := fs.MkdirAll(dir, defaultDirFileMode); err != nil {
