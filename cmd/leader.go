	...
	// Start servers
	{
		...
		if viper.GetBool("replication.enabled") {
			...
			replication, err := createReplicationServer(logger.Named("server.replication"), func(r grpc.ServiceRegistrar) {
				...
				mux.Handle("/snapshots-live/", armadaserver.NewLiveSnapshotHTTPHandler(engine, logger.Named("snapshot-http-live").Sugar()))
				if sharedStoreBucket != nil {
					mux.Handle("/snapshots/", store.NewSnapshotHTTPHandler(sharedStoreBucket, logger.Named("snapshot-http").Sugar()))
				}
			})
			if err != nil {
