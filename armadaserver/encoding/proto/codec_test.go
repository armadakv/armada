...
func TestCodec_Marshal(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			args: args{
				v: &armadapb.KeyValue{Key: []byte("key")},
			},
		},
		{
...
func TestCodec_Unmarshal(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			args: args{
				v:    &armadapb.KeyValue{},
				data: mustMarshall(&armadapb.KeyValue{Key: []byte("key")}),
			},
		},
		{
...
func BenchmarkCodec_Unmarshal(b *testing.B) {
	...
	for _, bm := range benchmarks {
		message := mustMarshall(&armadapb.TxnRequest{Success: []*armadapb.RequestOp{
			{
				Request: &armadapb.RequestOp_RequestPut{
					RequestPut: &armadapb.RequestOp_Put{Key: []byte("key1"), Value: []byte("value")},
				},
			},
			{
				Request: &armadapb.RequestOp_RequestPut{
					RequestPut: &armadapb.RequestOp_Put{Key: []byte("key2"), Value: []byte("value")},
				},
			},
			{
				Request: &armadapb.RequestOp_RequestPut{
					RequestPut: &armadapb.RequestOp_Put{Key: []byte("key3"), Value: []byte("value")},
				},
			},
			{
				Request: &armadapb.RequestOp_RequestPut{
					RequestPut: &armadapb.RequestOp_Put{Key: []byte("key4"), Value: []byte("value")},
				},
			},
			{
				Request: &armadapb.RequestOp_RequestPut{
					RequestPut: &armadapb.RequestOp_Put{Key: []byte("key5"), Value: []byte("value")},
				},
			},
		}})
...
func BenchmarkCodec_Marshal(b *testing.B) {
	...
	for _, bm := range benchmarks {
		message := &armadapb.RangeResponse{Kvs: []*armadapb.KeyValue{
