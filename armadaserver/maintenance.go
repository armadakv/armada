...
type ResetServer struct {
	armadapb.UnimplementedMaintenanceServer
	Tables   TableService
	AuthFunc func(ctx context.Context) (context.Context, error)
}

func (m *ResetServer) Reset(ctx context.Context, req *armadapb.ResetRequest) (*armadapb.ResetResponse, error) {
	...
	if req.ResetAll {
		...
		return &armadapb.ResetResponse{}, nil
	}
	if len(req.Table) == 0 {
...
func (m *BackupServer) Backup(req *armadapb.BackupRequest, srv armadapb.Maintenance_BackupServer) error {
	ctx := srv.Context()
...
func (m *BackupServer) Restore(srv armadapb.Maintenance_RestoreServer) error {
	msg, err := srv.Recv()
	if err != nil {
		return err
	}
	...
	if info.Format != "" && info.Format != "armada-command-v2" {
	...
	return srv.SendAndClose(&armadapb.RestoreResponse{})
}
...
type backupReader struct {
	stream armadapb.Maintenance_RestoreServer
}
