...
func (y *MemFS) iterate(node *memNode, f func(path string, isDir bool, refs int32) error) error {
	if !node.isDir {
		panic("not a dir")
	}
	for name, child := range node.children {
		if err := f(name, child.isDir, child.refs.Load()); err != nil {
			return err
		}
		if child.isDir {
...
func (f *memFile) Seek(offset int64, whence int) (int64, error) {
	...
	if whence == io.SeekEnd {
		if int64(len(f.n.mu.data))-offset < 0 {
			return 0, io.EOF
		}
		f.pos = len(f.n.mu.data) + int(offset)
		return offset, nil
	}
	return offset, nil
}
